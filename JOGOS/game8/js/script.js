(function(){
	...
	function init(){
		...
		const palavras = ['alcachofra', 'abacaxi', 'açai', 'caju', 'tomate','uva-verde',
		                  'tangerina','morango','nabo','alface','pera'];
		...
		for(var i = 1; i < 9; i++){
			...
			tile.style.background = "url('img/"+palavra+"/"+palavra+"_00"+ i +".jpg')";
			tile.addEventListener("click",moveTile,false);
			tiles.push(tile);
		}

		var fruta = document.querySelector(".img-flv");
			fruta.style.background = "url('img/"+palavra+"/"+palavra+".jpg')";
			fruta.style.backgroundSize = "contain";
    		fruta.style.backgroundRepeat = "no-repeat";
	...
	function render(){
		for(var i in tiles){
			var tile = tiles[i];
			if(tile){
				tile.style.left = (i%3) * 130 + 5 + "px";
				if(i<3){
					tile.style.top = "5px";
				} else 
				if(i<6){
					tile.style.top = "135px";
				} else {
					tile.style.top = "265px";
				}
